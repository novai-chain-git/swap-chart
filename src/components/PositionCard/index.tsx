...
export function MinimalPositionCard({ pair, showUnwrapped = false, border, background }: PositionCardProps) {
  const { account } = useActiveWeb3React()
  const { t } = useTranslation()
  const darkMode = useIsDarkMode()
  const themeMode = theme(darkMode)
  ...
  return (
    <>
      {userPoolBalance && (
        <GreyCard border={border} style={{ background: background || themeMode.bg2 }}>
          <AutoColumn gap="12px">
            <FixedHeightRow>
              <RowFixed>
