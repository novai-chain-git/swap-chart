...
function toReversedPolyfill<T>(arr: T[]): T[] {
  return [...arr].reverse()
}
export class StackedAreaSeriesRenderer<TData extends StackedAreaData> implements ICustomSeriesPaneRenderer {
  ...
  _drawImpl(renderingScope: BitmapCoordinatesRenderingScope, priceToCoordinate: PriceToCoordinateConverter): void {
    ...
    ctx.lineJoin = 'round'
    toReversedPolyfill(fullLinesMeshed).forEach((linePath, index) => {
      ...
      if (index !== fullLinesMeshed.length - 1) {
        ...
        const hoverY = toReversedPolyfill(hoverInfo.points)[index]
    ...
    toReversedPolyfill(highlightLinesMeshed).forEach((linePath, index) => {
      if (!linePath) {
        ctx.globalAlpha = 1
        return
      }
  ...
  _createAreas(linesMeshed: LinePathData[]): Path2D[] {
    ...
    if (linesMeshed.length < 2) return []

    for (let i = 1; i < linesMeshed.length; i++) {
      const baseReferenceIndex = i - 1
      const base = linesMeshed[baseReferenceIndex]
      const current = linesMeshed[i]
      if (!base || !current || !base.first || !current.first) continue

      const areaPath = new Path2D(base.path)
      areaPath.lineTo(current.first.x, current.first.y)
      areaPath.addPath(current.path)
      areaPath.lineTo(base.first.x, base.first.y)
      areaPath.closePath()
      areas.push(areaPath)
    }
    return areas
