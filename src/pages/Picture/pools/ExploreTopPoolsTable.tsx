import React, { NamedExoticComponent, memo, useState, ReactNode, useEffect, useMemo } from 'react'
...
export const ExploreTopPoolTable =({token}:{token: string}) => {
 ...
 // const [list, setList] = useState([])
  ...
  const getList = useMemo(()=>{
    if(!token) return []
    //console.log(token.toUpperCase(),'token.toUpperCase()')
      let list = dataSource.filter((item:any)=>{
        return item.lable?.toUpperCase() === token.toUpperCase()
      })
      return list
  },[token,dataSource])
  ...
  return (
    <TableComponent dataSource={getList} columns={columns}></TableComponent>
  )
}
